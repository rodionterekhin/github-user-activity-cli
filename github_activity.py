...
def print_event(event: Event) -> None:
    ...
    match event.type:
        ...
        case EventType.PUSH_EVENT:
            txt += (
                f"{event.payload.size} commits pushed to {event.payload.ref}"
            )
        case EventType.RELEASE_EVENT:
            txt += f"{event.payload.action} a release"
        case EventType.SPONSORSHIP_EVENT:
            txt += (
                f"{event.payload.action} aactivity related to sponsorship"
                " listing"
            )
